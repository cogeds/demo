...
function buildLogo(block) {
  ...
  return ` ${link?.href || '/'} ${image?.outerHTML || ''} </a> `;
}
function buildNavigation(block) {
  ...
  return ` <ul class="header-nav-list"> ${navLinks.map((link) => ` <li> ${link.href} ${link.textContent} </a> </li> `).join('')} </ul> `;
}
function buildUtilityNav(block) {
  ...
  return ` <ul class="header-utility-list"> ${links.map((link) => ` <li> ${link.href} </li> `).join('')} </ul> `;
}
...
export default async function decorate(block) {
  ...
  block.innerHTML = '';
  const header = createElement('header', { class: 'tmna-header' });
  header.innerHTML = ` <div class="header-container"> <button class="header-mobile-toggle" aria-label="Toggle Navigation" aria-expanded="false"> <span></span> <span></span> <span></span> </button> ${logoMarkup} <nav class="header-navigation" aria-label="Main navigation"> ${navMarkup} </nav> <div class="header-utility"> ${utilityMarkup} <button class="account-trigger" aria-label="Account"> Account </button> </div> </div> <div class="header-mega-menu"> <div class="mega-menu-content"> <div class="mega-column"> <h3>Vehicles</h3> <div id="vehicles-fragment"></div> </div> <div class="mega-column"> <h3>Shopping Tools</h3> <div id="shopping-tools-fragment"></div> </div> </div> </div> <div class="account-panel"> <h3>Personalize Your Toyota Experience</h3> <p> Create an account or sign in to access all the tools for your Toyota in one place. </p> <a href="/my-dashboard/">My Dashboard</a> </div> `;
  block.append(header);
