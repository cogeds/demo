...
async function decorateHeaderDefault(block) {
    ...
    const navLinks = [...fragment.querySelectorAll('a')].filter((link) => !link.querySelector('img'));
    const authoredH1 = document.querySelector('main h1');
    const pageTitleText = authoredH1 ? authoredH1.textContent.trim() : '';

    const header = buildHeader(logoImg?.cloneNode(true), navLinks, pageTitleText);
...
function accountRowHtml(li) {
    ...
    const meta = count ? `<span class="count">(${count})</span><span class="dot" aria-hidden="true"></span>` : '';
...
function decorateHeaderBrand(block) {
    ...
    function buildMegaGroup(li) {
        ...
        if (nestedList) {
            ...
            [...nestedList.children].forEach((item) => {
                const { label: itemLabelText, href: itemHref } = getLabelAndHref(item);
                const liItem = document.createElement('li');
                const aItem = document.createElement('a');
                aItem.href = itemHref;
                aItem.textContent = itemLabelText;
                liItem.append(aItem);
                list.append(liItem);
            });
            group.append(list);
    ...
    function triggerBlink(element) {
        ...
        element.addEventListener('animationend', () => element.classList.remove('blink-effect'), { once: true });
    }
...
function decorateHeaderFirm(block) {
    ...
    titleEl.textContent = titleLink ? titleLink.textContent.trim() : titleRow.textContent.trim();
    link.append(titleEl);
